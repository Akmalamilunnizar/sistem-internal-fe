  ...
  <div class="min-h-screen flex flex-col">
    ...
    <div v-if="showCreateModal || showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form @submit.prevent="showEditModal ? updateCustomerData() : createCustomerData()">
          <div class="space-y-4">
            <div>
...
const deleteCustomer = async (id: number) => {
  if (confirm('Are you sure you want to delete this customer?')) {
    try {
      await customerAdminApi().deleteCustomer(id)
      loadCustomers()
    } catch (error) {
...
const createCustomerData = async () => {
  try {
    await customerAdminApi().createCustomer(customerForm.value)
    closeModal()
    loadCustomers()
  } catch (error) {
...
const updateCustomerData = async () => {
  try {
    await customerAdminApi().updateCustomer(editingCustomer.value.id, customerForm.value)
    closeModal()
    loadCustomers()
  } catch (error) {
